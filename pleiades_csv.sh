...
echo "Accounts;Space persistent;Space temporary"
curl -s --capath /etc/grid-security/certificates "https://rucio-hadoop.cern.ch/datasets_per_rse?rse=UNI-BONN_LOCALGROUPDISK" | \
	sort -k4 | \
	awk '{SUM_TOT[$4]+=$5; if ($8 != "\"\"") {SUM_TMP[$4]+=$5} else {SUM_PST[$4]+=$5}} END { for (ACC in SUM_TOT) { printf("%s;%.3f;%.3f\n",ACC,SUM_PST[ACC]/1000/1000/1000/1000,SUM_TMP[ACC]/1000/1000/1000/1000) } }' | \
